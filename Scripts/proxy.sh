...
roxy() {
  ...
  echo "Proxy set in gnome settings successfully $proxy"
  echo -e "\ueba0 sudo -E apt install"
...
use_proxy() {
  ...
  echo "npm/pnpm/yarn proxy configurations set"
  echo "git global proxy configuration set"
}
...
unsetproxy() {
  ...
  echo "Proxy settings and environment variables unset"
  echo "npm/pnpm/yarn proxy configurations unset"
  echo "git global proxy configuration unset"
}
...
update_proxychain() {
  ...
  echo "Updating user-level proxychains config at $config_file"
...
EOF
  ...
  echo "Proxychains config updated: HTTP/HTTPS -> $proxy_ip:$http_port, SOCKS -> $proxy_ip:$sock_port"
}
